 ...
 * /MainCategory/beauty-wellness/book-appointment
 * Appointment booking flow: select salon/clinic, service, date/time
 * Beauty equivalent of food-dining's book-table
 */
...
const SERVICE_TYPES = [
  { id: 'haircut', label: 'Haircut', icon: '💇', duration: '30 min' },
  { id: 'facial', label: 'Facial', icon: '✨', duration: '45 min' },
  { id: 'manicure', label: 'Manicure', icon: '💅', duration: '30 min' },
  { id: 'pedicure', label: 'Pedicure', icon: '🦶', duration: '30 min' },
  { id: 'massage', label: 'Massage', icon: '💆', duration: '60 min' },
  { id: 'waxing', label: 'Waxing', icon: '🧴', duration: '30 min' },
  { id: 'makeup', label: 'Makeup', icon: '💄', duration: '60 min' },
  { id: 'bridal', label: 'Bridal', icon: '👰', duration: '120 min' },
];
...
function BookAppointmentPage() {
  ...
  const fetchSalons = useCallback(async () => {
    try {
      setIsLoading(true);
      const res = await storesApi.getStoresBySubcategorySlug('beauty-wellness', 50);
  ...
  const handleSubmit = async () => {
    if (!selectedStore?._id) { platformAlertSimple('Error', 'Please select a salon'); return; }
  ...
  const getServiceTags = (store: any): string => {
    if (store.tags?.length > 0) {
      const tags = store.tags
        .filter((t: string) => !['organic', 'cruelty-free', 'vegan', 'luxury'].includes(t.toLowerCase()))
        .slice(0, 3);
      if (tags.length) return tags.map((t: string) => t.charAt(0).toUpperCase() + t.slice(1)).join(' · ');
    }
    return store.category?.name || 'Beauty Services';
  };
          ...
          <View style={styles.storeImgWrap}>
            {imageUri ? (
              ...
              <View style={[styles.storeImg, styles.storeImgPlaceholder]}>
                <Ionicons name="flower" size={28} color={COLORS.textSecondary} />
              </View>
            )}
            {cashback ? (
      ...
      <SafeAreaView style={styles.container}>
        <View style={styles.header}>
          ...
          <View style={{ flex: 1 }}>
            ...
            <Text style={styles.headerSubtitle}>{salons.length} salons & clinics available</Text>
          </View>
        </View>
        ...
        <View style={styles.searchWrap}>
          <View style={styles.searchBar}>
            ...
            <TextInput
              ...
              placeholder="Search salons, clinics..."
              value={searchQuery}
        ...
        {isLoading ? (
          ...
          <View style={styles.emptyContainer}>
            ...
            <Text style={styles.emptyTitle}>
              {searchQuery ? 'No matches found' : 'No salons available'}
            </Text>
    ...
    return (
      <SafeAreaView style={styles.container}>
        <ScrollView contentContainerStyle={styles.confirmContent}>
          ...
          >
            <Text style={styles.doneBtnText}>Back to Beauty</Text>
          </Pressable>
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.formContent} showsVerticalScrollIndicator={false}>
            ...
            >
              ...
              <View style={{ flex: 1 }}>
                ...
                <Text style={styles.storePreviewMeta}>
                  {selectedStore.location?.city || 'Beauty services available'}
                </Text>
              </View>
              {!params.storeId && (
        ...
        {/* Service Selection */}
        <Text style={styles.formLabel}>Choose Service</Text>
        ...
        {/* Duration */}
        <Text style={{ fontSize: 14, fontWeight: '600', color: COLORS.textPrimary, marginTop: 16, marginBottom: 8 }}>Duration</Text>
        ...
        {/* Preferred Stylist */}
        <Text style={{ fontSize: 14, fontWeight: '600', color: COLORS.textPrimary, marginTop: 16, marginBottom: 8 }}>Preferred Stylist (Optional)</Text>
        <TextInput
          value={staffPreference}
          onChangeText={setStaffPreference}
          placeholder="Enter stylist name if you have a preference"
          placeholderTextColor={COLORS.textSecondary}
          style={{ borderWidth: 1, borderColor: COLORS.border, borderRadius: 12, paddingHorizontal: 14, paddingVertical: 10, fontSize: 14, color: COLORS.textPrimary, backgroundColor: COLORS.white }}
        />
        ...
        <TextInput
          ...
          placeholder="Special requests (allergies, preferred stylist, etc.)"
...
const styles = StyleSheet.create({
  ...
  bonusNote: {
    ...
    marginTop: 14, padding: 14, backgroundColor: COLORS.pinkLight, borderRadius: 14,
  },
  bonusIconWrap: {
    width: 28, height: 28, borderRadius: 8,
    backgroundColor: 'rgba(236,72,153,0.15)', justifyContent: 'center', alignItems: 'center',
  },
  bonusText: { flex: 1, fontSize: 12, color: COLORS.pinkDark, lineHeight: 17 },
