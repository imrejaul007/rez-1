// @ts-nocheck
/**
 * Book a Doctor / Appointment Page
 * /MainCategory/healthcare/book-doctor
 * Booking flow: Select Specialty -> Choose Doctor -> Pick Time -> Describe Symptoms -> Confirm
 * Healthcare equivalent of beauty-wellness's book-appointment
 */
...
const SPECIALTIES = [
  { id: 'general', label: 'General Physician', icon: '👨‍⚕️', duration: '20 min' },
  { id: 'cardiology', label: 'Cardiology', icon: '❤️', duration: '30 min' },
  { id: 'dermatology', label: 'Dermatology', icon: '🧴', duration: '20 min' },
  { id: 'orthopedic', label: 'Orthopedic', icon: '🦴', duration: '25 min' },
  { id: 'pediatric', label: 'Pediatric', icon: '👶', duration: '20 min' },
  { id: 'gynecology', label: 'Gynecology', icon: '🤰', duration: '25 min' },
  { id: 'ent', label: 'ENT', icon: '👂', duration: '20 min' },
  { id: 'ophthalmology', label: 'Ophthalmology', icon: '👁️', duration: '20 min' },
];
...
function BookDoctorPage() {
  ...
  const [selectedSpecialty, setSelectedSpecialty] = useState(params.service || '');
  ...
  const fetchClinics = useCallback(async () => {
    try {
      setIsLoading(true);
      const res = await storesApi.getStoresBySubcategorySlug('healthcare', 50);
  ...
  useEffect(() => {
    if (!params.storeId) fetchClinics();
  }, [fetchClinics, params.storeId]);
  ...
  const handleSelectClinic = (store: any) => {
  ...
  const handleSubmit = async () => {
    if (!selectedStore?._id) { platformAlertSimple('Error', 'Please select a clinic or hospital'); return; }
    if (!selectedSpecialty) { platformAlertSimple('Error', 'Please select a specialty'); return; }
    ...
    try {
      ...
      if (res.success) {
        ...
        platformAlertSimple('Booking Failed', res.message || 'Could not create appointment. Please try again.');
      }
    } catch (err: any) {
  ...
  const getServiceTags = (store: any): string => {
    ...
    return store.category?.name || 'Healthcare Services';
  };
  ...
  if (step === 'clinic') {
    const renderStoreCard = ({ item: store }: { item: any }) => {
      ...
      return (
        <Pressable style={styles.storeCard} onPress={() => handleSelectClinic(store)}>
          <View style={styles.storeImgWrap}>
            {imageUri ? (
              ...
              <View style={[styles.storeImg, styles.storeImgPlaceholder]}>
                <Ionicons name="medkit" size={28} color={COLORS.textSecondary} />
              </View>
            )}
          ...
          <View style={styles.storeInfo}>
            <View style={styles.storeNameRow}>
              ...
              {isVerified && <View style={styles.verifiedBadge}><Text style={styles.verifiedBadgeText}>Verified</Text></View>}
            </View>
      ...
      <SafeAreaView style={styles.container}>
        <View style={styles.header}>
          ...
          <View style={{ flex: 1 }}>
            ...
            <Text style={styles.headerSubtitle}>{clinics.length} clinics & hospitals available</Text>
          </View>
        </View>
        ...
        <View style={styles.searchWrap}>
          <View style={styles.searchBar}>
            ...
            <TextInput style={styles.searchInput} placeholder="Search clinics, hospitals..." value={searchQuery} onChangeText={setSearchQuery} placeholderTextColor={COLORS.textSecondary} />
        ...
        {isLoading ? (
          <View style={styles.loadingContainer}>
            ...
            <Text style={styles.loadingText}>Finding clinics & hospitals...</Text>
          </View>
        ) : filteredClinics.length === 0 ? (
          <View style={styles.emptyContainer}>
            <Ionicons name="medkit-outline" size={48} color={COLORS.border} />
            <Text style={styles.emptyTitle}>{searchQuery ? 'No matches found' : 'No clinics available'}</Text>
  ...
  // ---- STEP 3: Confirmation ----
  if (step === 'confirm') {
    const specialtyName = SPECIALTIES.find(s => s.id === selectedSpecialty)?.label || selectedSpecialty;
    return (
      <SafeAreaView style={styles.container}>
        <ScrollView contentContainerStyle={styles.confirmContent}>
          <View style={styles.confirmIconWrap}>
            <LinearGradient colors={[COLORS.blueLight, '#E0F2FE']} style={styles.confirmIconGradient}>
              <Ionicons name="checkmark-circle" size={64} color={COLORS.blue} />
            </LinearGradient>
          </View>
          <Text style={styles.confirmTitle}>Appointment Booked!</Text>
          <Text style={styles.confirmSubtitle}>Your doctor appointment has been scheduled</Text>
          {bookingNumber && (
          ...
          <View style={styles.confirmCard}>
            <View style={styles.confirmRow}>
              <View style={[styles.confirmRowIcon, { backgroundColor: 'rgba(14,165,233,0.1)' }]}>
                <Ionicons name="medkit" size={16} color={COLORS.blue} />
              </View>
              <View>
                <Text style={styles.confirmRowLabel}>Clinic / Hospital</Text>
            ...
            <View style={styles.confirmRow}>
              ...
              <View>
                ...
                <Text style={styles.confirmRowValue}>{specialtyName}</Text>
              </View>
            </View>
            <View style={styles.confirmDivider} />
            <View style={styles.confirmRow}>
          ...
          <View style={styles.confirmNote}>
            ...
            <Text style={styles.confirmNoteText}>No pre-payment required. Earn bonus coins when you visit the clinic!</Text>
          </View>

          <Pressable style={styles.doneBtn} onPress={() => router.back()}>
            <Text style={styles.doneBtnText}>Back to Healthcare</Text>
          </Pressable>
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        <Pressable onPress={() => params.storeId ? router.back() : setStep('clinic')} style={styles.backBtn}>
          <Ionicons name="arrow-back" size={24} color={COLORS.textPrimary} />
        </Pressable>
        <View style={{ flex: 1 }}>
      ...
      <ScrollView contentContainerStyle={styles.formContent} showsVerticalScrollIndicator={false}>
        {selectedStore?.name && (
          <View style={styles.storePreview}>
            <LinearGradient colors={[COLORS.blue, COLORS.blueDark]} style={styles.storePreviewGradient} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }}>
              <View style={styles.storePreviewIcon}><Ionicons name="medkit" size={20} color={COLORS.white} /></View>
              <View style={{ flex: 1 }}>
                <Text style={styles.storePreviewName} numberOfLines={1}>{selectedStore.name}</Text>
                <Text style={styles.storePreviewMeta}>{selectedStore.location?.city || 'Healthcare services available'}</Text>
              </View>
              {!params.storeId && <Pressable onPress={() => setStep('clinic')}><Text style={styles.storePreviewChange}>Change</Text></Pressable>}
            </LinearGradient>
          </View>
        )}

        {/* Specialty Selection */}
        <Text style={styles.formLabel}>Select Specialty</Text>
        <View style={styles.serviceGrid}>
          {SPECIALTIES.map(spec => {
            const isSelected = selectedSpecialty === spec.id;
            return (
              <Pressable key={spec.id} style={[styles.serviceChip, isSelected && styles.serviceChipActive]} onPress={() => setSelectedSpecialty(spec.id)}>
                <Text style={styles.serviceEmoji}>{spec.icon}</Text>
                <Text style={[styles.serviceLabel, isSelected && styles.serviceLabelActive]}>{spec.label}</Text>
                <Text style={[styles.serviceDuration, isSelected && styles.serviceDurationActive]}>{spec.duration}</Text>
              </Pressable>
            );
          })}
        </View>
        ...
        <View style={styles.formLabelRow}>
          ...
          {isLoadingAvailability ? <ActivityIndicator size="small" color={COLORS.blue} /> : <Text style={styles.availableLabel}>{availableCount} slots available</Text>}
        </View>
        ...
        {/* Symptoms */}
        <Text style={styles.formLabel}>Describe Symptoms (Optional)</Text>
        <TextInput
        ...
        {selectedTime && selectedSpecialty ? (
          <View style={styles.bookingSummary}>
            <Text style={styles.summaryTitle}>Appointment Summary</Text>
            <View style={styles.summaryRow}>
              <Text style={styles.summaryLabel}>{SPECIALTIES.find(s => s.id === selectedSpecialty)?.label} \u00B7 {formatDate(selectedDate)} at {selectedTime}</Text>
            </View>
          </View>
        ) : null}

        <Pressable
          style={[styles.submitBtn, (isSubmitting || !selectedTime || !selectedSpecialty) && styles.submitBtnDisabled]}
          onPress={handleSubmit}
          disabled={isSubmitting || !selectedTime || !selectedSpecialty}
        ...
        <View style={styles.bonusNote}>
          <View style={styles.bonusIconWrap}><Ionicons name="wallet-outline" size={14} color={COLORS.blue} /></View>
          <Text style={styles.bonusText}>No pre-payment required. Earn bonus coins when you visit the clinic!</Text>
        </View>
      </ScrollView>
    </SafeAreaView>
  );
...
const styles = StyleSheet.create({
  ...
  verifiedBadge: { backgroundColor: COLORS.blueLight, borderRadius: 4, paddingHorizontal: 5, paddingVertical: 1 },
  verifiedBadgeText: { fontSize: 9, fontWeight: '600', color: COLORS.blue },
  ...
  serviceChipActive: { backgroundColor: COLORS.blueLight, borderColor: COLORS.blue },
  serviceEmoji: { fontSize: 20 },
  serviceLabel: { fontSize: 13, fontWeight: '500', color: COLORS.textPrimary, flex: 1 },
  serviceLabelActive: { color: COLORS.blueDark, fontWeight: '600' },
  serviceDuration: { fontSize: 10, color: COLORS.textSecondary },
  serviceDurationActive: { color: COLORS.blue },
  ...
  submitBtn: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, backgroundColor: COLORS.blue, borderRadius: 16, paddingVertical: 16, marginTop: 20 },
  submitBtnDisabled: { opacity: 0.5 },
  submitBtnText: { fontSize: 16, fontWeight: '600', color: COLORS.white },
  bonusNote: { flexDirection: 'row', alignItems: 'center', gap: 10, marginTop: 14, padding: 14, backgroundColor: COLORS.blueLight, borderRadius: 14 },
  bonusIconWrap: { width: 28, height: 28, borderRadius: 8, backgroundColor: 'rgba(14,165,233,0.15)', justifyContent: 'center', alignItems: 'center' },
  bonusText: { flex: 1, fontSize: 12, color: COLORS.blueDark, lineHeight: 17 },
  ...
  viewBookingsBtnText: { fontSize: 14, fontWeight: '600', color: COLORS.blue },
});

export default React.memo(BookDoctorPage);
