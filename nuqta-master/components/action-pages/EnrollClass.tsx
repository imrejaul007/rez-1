// @ts-nocheck
/**
 * Enroll in a Class/Course Page
 * /MainCategory/education-learning/enroll-class
 * Enrollment flow: Select Subject -> Choose Institute -> Pick Batch -> Schedule -> Confirm
 * Education equivalent of fitness book-class
 */
...
const SUBJECT_TYPES = [
  { id: 'math', label: 'Math', icon: '📐', duration: '60 min' },
  { id: 'science', label: 'Science', icon: '🔬', duration: '60 min' },
  { id: 'english', label: 'English', icon: '📖', duration: '45 min' },
  { id: 'coding', label: 'Coding', icon: '💻', duration: '90 min' },
  { id: 'music', label: 'Music', icon: '🎵', duration: '45 min' },
  { id: 'dance', label: 'Dance', icon: '💃', duration: '60 min' },
  { id: 'art', label: 'Art', icon: '🎨', duration: '60 min' },
  { id: 'languages', label: 'Languages', icon: '🌍', duration: '45 min' },
];
...
function EnrollClassPage() {
  ...
  const fetchSalons = useCallback(async () => {
    try {
      setIsLoading(true);
      const res = await storesApi.getStoresBySubcategorySlug('education-learning', 50);
  ...
  const handleSelectSalon = (store: any) => { setSelectedStore(store); setStep('details'); };
  ...
  const handleSubmit = async () => {
    if (!selectedStore?._id) { platformAlertSimple('Error', 'Please select an institute'); return; }
    if (!selectedService) { platformAlertSimple('Error', 'Please select a subject'); return; }
  ...
  const getServiceTags = (store: any): string => {
    if (store.tags?.length > 0) {
      const tags = store.tags
        .filter((t: string) => !['online', 'certified', 'budget', 'open-now'].includes(t.toLowerCase()))
        .slice(0, 3);
      if (tags.length) return tags.map((t: string) => t.charAt(0).toUpperCase() + t.slice(1)).join(' \u00B7 ');
    }
    return store.category?.name || 'Education Services';
  };
  ...
  if (step === 'salon') {
    const renderStoreCard = ({ item: store }: { item: any }) => {
      ...
      return (
        <Pressable style={styles.storeCard} onPress={() => handleSelectSalon(store)}>
          <View style={styles.storeImgWrap}>
            {imageUri ? (
              ...
              <View style={[styles.storeImg, styles.storeImgPlaceholder]}>
                <Ionicons name="school" size={28} color={COLORS.textSecondary} />
              </View>
            )}
            {cashback ? (
    ...
    return (
      <SafeAreaView style={styles.container}>
        <View style={styles.header}>
          ...
          <View style={{ flex: 1 }}>
            <Text style={styles.headerTitle}>Enroll in a Class</Text>
            <Text style={styles.headerSubtitle}>{salons.length} institutes available</Text>
          </View>
        </View>
        ...
        <View style={styles.searchWrap}>
          <View style={styles.searchBar}>
            ...
            <TextInput
              ...
              placeholder="Search institutes, tutors..."
              value={searchQuery}
        ...
        {isLoading ? (
          ...
          <View style={styles.emptyContainer}>
            ...
            <Text style={styles.emptyTitle}>{searchQuery ? 'No matches found' : 'No institutes available'}</Text>
  ...
  // --- STEP 3: Confirmation ---
  if (step === 'confirm') {
    const serviceName = SUBJECT_TYPES.find(s => s.id === selectedService)?.label || selectedService;
    return (
      <SafeAreaView style={styles.container}>
        <ScrollView contentContainerStyle={styles.confirmContent}>
          ...
          <Text style={styles.confirmSubtitle}>Your class has been scheduled</Text>
          {bookingNumber && (
            <View style={styles.bookingIdWrap}>
              <Text style={styles.bookingIdLabel}>Enrollment Reference</Text>
          ...
          <View style={styles.confirmCard}>
            ...
            <View style={styles.confirmRow}>
              <View style={[styles.confirmRowIcon, { backgroundColor: 'rgba(139,92,246,0.1)' }]}>
                <Ionicons name="book" size={16} color={colors.brand.purpleLight} />
              </View>
              <View>
                <Text style={styles.confirmRowLabel}>Subject</Text>
          ...
          <Pressable style={styles.doneBtn} onPress={() => router.back()}>
            <Text style={styles.doneBtnText}>Back to Education</Text>
          </Pressable>

          <Pressable style={styles.viewBookingsBtn} onPress={() => router.push('/orders' as any)}>
            <Text style={styles.viewBookingsBtnText}>View My Enrollments</Text>
          </Pressable>
        </ScrollView>
      </SafeAreaView>
    );
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        <Pressable onPress={() => params.storeId ? router.back() : setStep('salon')} style={styles.backBtn}>
          <Ionicons name="arrow-back" size={24} color={COLORS.textPrimary} />
        </Pressable>
        <View style={{ flex: 1 }}>
          <Text style={styles.headerTitle}>Enroll in Class</Text>
      ...
      <ScrollView contentContainerStyle={styles.formContent} showsVerticalScrollIndicator={false}>
          ...
          <View style={styles.storePreview}>
            <LinearGradient colors={[COLORS.indigo, COLORS.indigoDark]} style={styles.storePreviewGradient} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }}>
              ...
              <View style={{ flex: 1 }}>
                ...
                <Text style={styles.storePreviewMeta}>{selectedStore.location?.city || 'Education services available'}</Text>
              </View>
              {!params.storeId && (
                <Pressable onPress={() => setStep('salon')}><Text style={styles.storePreviewChange}>Change</Text></Pressable>
              )}
            </LinearGradient>
          </View>
        )}
        ...
        {/* Subject Selection */}
        <Text style={styles.formLabel}>Choose Subject</Text>
        ...
        <TextInput
          ...
          placeholder="Special requests (learning level, goals, etc.)"
          value={specialRequests} onChangeText={setSpecialRequests}
          multiline numberOfLines={3} placeholderTextColor={COLORS.textSecondary}
        />
          ...
          <View style={styles.bookingSummary}>
            ...
            <View style={styles.summaryRow}>
              <Text style={styles.summaryLabel}>
                {SUBJECT_TYPES.find(s => s.id === selectedService)?.label} \u00B7 {formatDate(selectedDate)} at {selectedTime}
              </Text>
            </View>
          </View>
        ) : null}
...
const styles = StyleSheet.create({
  ...
  bonusNote: {
    ...
    marginTop: 14, padding: 14, backgroundColor: COLORS.indigoLight, borderRadius: 14,
  },
  bonusIconWrap: { width: 28, height: 28, borderRadius: 8, backgroundColor: 'rgba(99,102,241,0.15)', justifyContent: 'center', alignItems: 'center' },
  bonusText: { flex: 1, fontSize: 12, color: COLORS.indigoDark, lineHeight: 17 },
