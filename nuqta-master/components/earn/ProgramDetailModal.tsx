 ...
 * ProgramDetailModal
 *
 * Full-screen modal that shows Special Program detail.
 * Opens within the tab context (preserves navigation stack).
 *
 * Renders different views based on eligibility state:
 * - not_eligible: Requirements checklist
 * - eligible: Benefits + "Activate Now"
 * - pending_verification: Status + timeline
 * - active_member: Dashboard with earnings meter
 * - suspended/expired/revoked: Status message
 */
...
interface ProgramDetailModalProps {
  visible: boolean;
  onClose: () => void;
  programSlug: SpecialProgramSlug | null;
  onStatusChange?: () => void;
}
...
export const ProgramDetailModal: React.FC<ProgramDetailModalProps> = ({
  ...
  }, [visible, programSlug, fetchEligibility]);
  ...
  const renderActiveMember = () => {
    ...
    return (
      <ScrollView style={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        {/* How to Earn */}
        {membership.earningActivities && membership.earningActivities.length > 0 && (
          <View style={styles.card}>
            <Text style={styles.cardTitle}>How to Earn</Text>
            <Text style={[styles.cardSubtitle, { marginBottom: Spacing.md }]}>
              Activities that earn you coins with your {membership.multiplier > 1 ? `${membership.multiplier}x` : ''} multiplier
            </Text>
            {membership.earningActivities.map((activity, idx) => (
              <View key={idx} style={styles.earningActivityRow}>
                <Text style={styles.earningActivityIcon}>{activity.icon}</Text>
                <View style={styles.earningActivityText}>
                  <Text style={styles.earningActivityTitle}>{activity.label}</Text>
                  <Text style={styles.earningActivityDesc}>{activity.description}</Text>
                </View>
              </View>
            ))}
          </View>
  ...
  const renderNotEligible = () => (
    <ScrollView style={styles.scrollContent} showsVerticalScrollIndicator={false}>
      ...
      {eligibility!.verificationRejected && (
        <View style={[styles.card, styles.rejectionCard]}>
          ...
          <Text style={styles.rejectionReason}>
            {eligibility!.rejectionReason || 'Your previous verification was not approved. Please re-submit with valid documents.'}
          </Text>
        </View>
      )}
      ...
      {/* Verify CTA (for student/corporate) */}
      {!isPrive && eligibility!.requirements.some(r => !r.met && r.type === 'verification') && (
        <Pressable
          style={[styles.activateButton, { backgroundColor: iconConfig.color, flexDirection: 'row', gap: Spacing.sm }]}
  ...
  const renderInactive = () => (
    <View style={styles.centerContent}>
      ...
      {(eligibility!.state === 'suspended' || eligibility!.state === 'expired') && !isPrive && (
        <Pressable
          style={[styles.activateButton, { backgroundColor: iconConfig.color, marginTop: Spacing.lg, flexDirection: 'row', gap: Spacing.sm }]}
          onPress={handleVerify}
