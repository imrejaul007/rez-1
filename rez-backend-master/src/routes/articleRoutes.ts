...
// Get all articles with filtering (cached - public, read-heavy)
router.get(
  '/',
  optionalAuth,
  cacheMiddleware({ ttl: CacheTTL.STATIC_DATA, keyPrefix: 'articles:list', condition: () => true }),
  validateQuery(
    Joi.object({
...
// Get trending articles (cached - public, based on engagement metrics)
router.get(
  '/trending',
  optionalAuth,
  cacheMiddleware({ ttl: CacheTTL.PRODUCT_FEATURED, keyPrefix: 'articles:trending', condition: () => true }),
  validateQuery(
    Joi.object({
...
// Get featured articles (cached - public, editorially selected)
router.get(
  '/featured',
  optionalAuth,
  cacheMiddleware({ ttl: CacheTTL.CATEGORY_LIST, keyPrefix: 'articles:featured', condition: () => true }),
  validateQuery(
    Joi.object({
      limit: Joi.number().integer().min(1).max(50).default(10),
    }),
...
// Get articles by category (cached - public)
router.get(
  '/category/:category',
  optionalAuth,
  cacheMiddleware({ ttl: CacheTTL.STATIC_DATA, keyPrefix: 'articles:category', condition: () => true }),
  validateParams(
    Joi.object({
      category: Joi.string().valid('fashion', 'beauty', 'lifestyle', 'tech', 'general').required(),
    }),
...
// Record article view (authenticated or anonymous)
router.post(
  '/:articleId/view',
  optionalAuth,
  validateParams(
