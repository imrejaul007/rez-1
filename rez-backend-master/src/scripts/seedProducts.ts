...
const productsToSeed = [
  // New Arrivals Data
  {
    ...
    description:
      'Scented candles for relaxation and ambiance. Perfect for creating a peaceful atmosphere in your home.',
  ...
  // Just for You (Recommendations) Data
  {
    ...
    description:
      'Comfortable ergonomic chair for home office. Perfect for long working hours with lumbar support and adjustable height.',
  ...
  },
  {
    ...
    description:
      "Premium running shoes for daily training. Designed for comfort and performance with Nike's latest technology.",
...
const createDefaultData = async () => {
  ...
  // OPTIMIZATION: Use bulkWrite for category upserts instead of sequential findOneAndUpdate
  // bulkWrite sends a single batch operation to MongoDB, significantly reducing round trips
  ...
  try {
    const categoryResult = await Category.bulkWrite(categoryBulkOps, { ordered: false });
    console.log(
      `📦 Category bulkWrite: ${categoryResult.upsertedCount} inserted, ${categoryResult.modifiedCount} updated`,
    );
  } catch (error) {
    // Handle bulk write errors gracefully - continue even if some operations fail
    console.warn('⚠️ Some category bulk operations failed:', error);
  }

  // OPTIMIZATION: Use bulkWrite for store upserts instead of sequential findOneAndUpdate
  // unordered bulk writes allow parallel execution for better performance
  const storeBulkOps = stores.map((store) => ({
    updateOne: {
  ...
  try {
    const storeResult = await Store.bulkWrite(storeBulkOps, { ordered: false });
    console.log(`📦 Store bulkWrite: ${storeResult.upsertedCount} inserted, ${storeResult.modifiedCount} updated`);
  } catch (error) {
    // Handle bulk write errors gracefully - continue even if some operations fail
    console.warn('⚠️ Some store bulk operations failed:', error);
  }
...
const seedProducts = async () => {
  try {
    ...
    // OPTIMIZATION: Use insertMany for bulk product insertion instead of individual saves
    // This reduces N database round trips to a single bulk insert operation
    const productDocs = productsToSeed.map((productData) => {
    ...
    try {
      ...
      console.log(`📦 Bulk inserted ${insertedProducts.length} products using insertMany`);
    } catch (error) {
      // Handle partial failure - some products may have been inserted
      console.error('⚠️ Error during bulk product insertion:', error);
    }
